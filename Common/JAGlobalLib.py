...
"""
def LogLine(myLines, tempPrintLine, myColors, colorIndex, outputFile, HTMLBRTag):
...
def JAExecuteCommand(command, OSType, OSName, OSVersion, debugLevel):
    """
...
def JADeriveHistoryFileName( subsystem, operation, defaultParameters):
    historyFileName = "{0}JAAudit.{0}.{1}.PrevStartTime".format(defaultParameters['LogFilePath'], operation,subsystem)
    return historyFileName

def JAIsItTimeToRunOperation(currentTime, subsystem, operation, defaultParameters, debugLevel):
    """
...
def JADeriveConfigFileName( pathName, prefix, baseConfigFileName, subsystem, appVersion, debugLevel ):
    """
...
def JACheckConnectivity( hostName, port, protocol, command, tcpOptions, udpOptions, OSType, OSName, OSVersion, debugLevel):
    """
...
def JACheckConnectivityToHosts( defaultParameters, connectivitySpec, OSType, OSName, OSVersion, printResults, debugLevel):
    ...
    for hostSpec in connectivitySpec:
        ...
        protocol = hostSpec[1]
        port = hostSpec[2]
        tempReturnStatus,returnOutput, errorMsg = JACheckConnectivity( hostName, port, protocol, command, tcpOptions, udpOptions, OSType, OSName, OSVersion, debugLevel)
