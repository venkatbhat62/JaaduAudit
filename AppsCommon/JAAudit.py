...
# max time in seconds allowed to execute commands, applies to forked jobs
totalWaitTime = 600 
totalWaitTimeForStats = 1200
...
def JAHelp():
    helpString1 = """
    python JAAudit.py -o <operations> [-s <subsystem>] [-p <repositoryName>] [-k <KITSHostName>] [-H <downloadHostName>] 
    ...
    -o <operations> - can have one or more operations in CSV format. operations supported are -
        ...
        If called with operation 'default'
            ...
            This approach allows single crontab entry to run diverse set of operations. Those operations
                spec can be changed in a single source on KITS or git type of source repository and make the 
        ...
        If called with operation 'discover'
            ...
            Typically used to replace previous file references in save<baseConfigFile><subsystem>.yml with current file references
              ...
              update the spec file on KITS or git type of repository.

        If called with operation 'download'
            If rsync is enabled for that host, it will sync files from KITS host to local host from specified path.
            If not, uses wget to get latest files from KITS host to local host from specified path.
            Typically used during host to host compare, or to restore the files backed up before on KITS host. 
        ...
        If called with operation 'sync'
            Syncs the code & config (all spec files) from KITS to local host. rsync will be used if specified
        ...
        If called with operation 'task'
            ...
            Typically used to manage tasks to be run on many hosts of one or more environments by using
                single definition source on KITS or git type of repository. This task approach eliminates the
        ...
        If called with operation 'upload'
            Uploads the files from JAAudit/<saveDir> to KITS host and saved under 
                <WebServerMainDir>/<platform>/<hostName>/
                <WebServerAltDir>/<platform>/<hostName>/
            Typically done for host to host compare or to take backup of files from target host on KITS host
            Operations like inventory, cert, conn, stats, health, test do implicit upload to upload the data collected
               to KITS host for further processing when such operations are executed via non-interactive session.
    ...
    [-p <repositoryName>] - platform name to be sent to KITS host while doing rsync or wget so that
        platform specific files are downloaded to current host. This platform name is also used
        while uploading the file to KITS so that the file is placed under platform specific path on KITS host.
        The directory or folder path on KITS is computed using <WebServerMain>/<platform>/<hostname> and
            <WebServerAlt>/<platform>/<hostname>
        Default value is picked up from environment configuration file for the current host.

    [-k <KITSHostName>] - KITS hostname used for upload and download operations.
    ...
    [-d <saveDirectory>] - Directory where files will be saved if operation is 'save' or 'download'.
        Directory from where files will be uploaded to KITS if operation is 'upload'
    ...
    Examples:
        ...
        python JAAudit.py -o save,upload <-- save current environment and upload to KITS host, 
                typically done before code install. 
            Default <saveDir> is used. (see <saveDir> for more details)
            Default <subsystem> of 'App' is used (see <subsystem> for more details)
            Default base config file name specified in environment config file is used for the current hostname
            Default KITS hostname is used (see <KITSHostName> for more details)

        python JAAudit.py -o save,upload -D 2 <-- save current environment and upload to KITS host, 
                typically done before code install. 
            Default <saveDir> is used. (see <saveDir> for more details)
            Default <subsystem> of 'App' is used (see <subsystem> for more details)
            Default base config file name specified in environment config file is used for the current hostname
        ...
        python JAAudit.py -o download,compare -k KITS1 -H RefHostName <-- download environment from KITS1 
                for the hostname RefHostName and compare to current host, typically done to perform host to host complare. 
            Default <saveDir> is used. (see <saveDir> for more details)
            Default <subsystem> of 'App' is used (see <subsystem> for more details)
            Default base config file name specified in environment config file is used for the current hostname

        python JAAudit.py -o download,compare -k KITS1 -H RefHostName -i yes <-- download environment from KITS1 
        ...
        python JAAudit.py -o sync <-- sync code and config from KITS to current host
                Typically used to update the Audit tool artifact on a host where periodic sync is not enabled

        python JAAudit.py -o inventory,upload -k KITS1 <-- take inventory and upload to KITS1 
        ...
        python JAAudit.py -o health,upload <-- collect health and upload to KITS
...
if '-k' in argsPassed:
    # KITS host name passed
    KITSHostName = argsPassed['-k']
else:
    KITSHostName = ''
...
errorMsg  = "JAAudit.py Version:{0}, OSType: {1}, OSName: {2}, OSVersion: {3}".format(JAVersion, OSType, OSName, OSVersion)
print(errorMsg)
...
### if KITS hostname is passed, use that for sync, download, upload operations
if KITSHostName != '':
    defaultParameters['KITSHostName'] = defaultParameters['KITSHostName1'] = defaultParameters['KITSHostName2']  = KITSHostName
...
command = 'find {0} -name "JAAudit*.log.*" -mtime +{1} |xargs rm'.format(logFilePath, fileRetencyDurationInDays)
if debugLevel > 0:
...
if 'CommandToGetAppVersion' in defaultParameters:
    ...
    returnResult, returnOutput, errorMsg = JAGlobalLib.JAExecuteCommand(commandToGetAppVersion, OSType, OSName, OSVersion, debugLevel)
...
### if operation is default, sync, download, or upload, need to connect to KITS. Check the connectivity
if re.match(r"sync|download|upload|default", operations):

    if 'KITSHostName1' in defaultParameters and 'KITSPortHTTPS' in defaultParameters:
        # check connectivity to KITS host
        connectivitySpec = [
            [defaultParameters['KITSHostName1'], 'TCP', defaultParameters['KITSPortHTTPS']],
            ]
        ...
        if passCount > 0:
            prevKitsHostCheckStatus = True
            KITSHostName = defaultParameters['KITSHostName1']
        else:
            # check connectivity to KITSHostName2
            connectivitySpec = [
                [defaultParameters['KITSHostName2'], 'TCP', defaultParameters['KITSPortHTTPS']],
                ]
            returnStatus, passCount, failureCount, detailedResults = JAGlobalLib.JACheckConnectivityToHosts( 
               defaultParameters, connectivitySpec, OSType, OSName, OSVersion, interactiveMode, debugLevel)
            if passCount > 0:
                prevKitsHostCheckStatus = True
                KITSHostName = defaultParameters['KITSHostName2']
            else:
                # KITS hostname is not accessible
                KITSHostName = None
        ...
        if ( (re.match(r"sync", operations) != None and  
            ...
            if 'KITSPortRsync' in defaultParameters:
                # check connectivity to KITS host
                connectivitySpec = [
                    [KITSHostName, 'TCP', defaultParameters['KITSPortRsync']],
                    ]
                returnStatus, passCount, failureCount, detailedResults = JAGlobalLib.JACheckConnectivityToHosts( 
                    defaultParameters, connectivitySpec, OSType, OSName, OSVersion, interactiveMode, debugLevel)
                if passCount != 1:
                    print("ERROR JAAudit() connectivity check to rsync port:{0} on KITSHostName:{1} FAILED".format(
                                defaultParameters['KITSPortRsync'], KITSHostName ))
                else:
                    if debugLevel > 0:
                        print("DEBUG-1 JAAudit() connectivity check to rsync port:{0} on KITSHostName:{1} PASSED".format(
                                defaultParameters['KITSPortRsync'], KITSHostName
                            ))


### if sync is opted, run it
if re.match("sync", operations) != None and re.match("nosync", operations) == None:
...
###    upload results to KITS
if re.match("conn", operations):
